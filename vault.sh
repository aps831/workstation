#! /bin/bash
set -e
vault_nas_titan_password="$(pass odin.local/titan/password)"
vault_nas_titan_password_b64="$(echo "${vault_nas_titan_password}" | base64)"
vault_nas_thor_password="$(pass odin.local/titan/password)" # Currently no thor user set up
vault_nas_thor_password_b64="$(echo "${vault_nas_thor_password}" | base64)"
...
temp_file=$(mktemp)
cat <<EOF >"${temp_file}"
vault_nas_titan_password: "${vault_nas_titan_password_b64}"
vault_nas_thor_password: "${vault_nas_thor_password_b64}"
